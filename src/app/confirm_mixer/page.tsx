...
export default function ConfirmMixer() {
  const { queryParams, setQueryParams } = useQueryParams<{
    receiver_address: string;
    delay: string;
  }>();
  const { MixerDetails, setMixerDetails } = useMixerDetails();
  const { delay, receiver_address } = MixerDetails;
  ...
  useEffect(() => {
    ...
    if (!isReceiverValid && !isReceiverFromRouteValid) {
      ...
      const receiver_address_to_use = isReceiverValid
        ? receiver_address
        : (receiver_address_from_route as string);
