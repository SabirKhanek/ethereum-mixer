...
export function Modal(props: ModalProps) {
  ...
  const handleContinue = () => {
    if (isValidBEP20Address(receiver_address)) {
      const path = `confirm_mixer${createSearchParams({
        receiver_address: receiver_address,
        delay: selectedOption.id,
      })}`;

      mixerDetailsContext.setMixerDetails({
        receiver_address,
        delay: Number.parseInt(selectedOption.id),
      });
      router.push(path);
    } else setError(true);
  };
      ...
      <div
        ...
        <div className="my-2 mx-3 border border-black p-5 relative bg-[#E6E6E6]">
          ...
          <div className="font-epilogue mt-4">
            <label htmlFor="receiver_address" className="uppercase text-base">
              Enter wallet address to receive mixed eth
            </label>
            <input
              value={receiver_address}
              onChange={(e) => setreceiverAddress(e.target.value)}
              name="receiver_address"
              className="border border-gray-300 bg-white text-grey ring-slate-600 w-full text-lg py-2.5 px-2 mb-0"
              placeholder="Example: 0xD8DA6BF26964AF9D7EED9E03E53415D37AA96045"
              type="text"
            />
            {error && (
              ...
              {showDropdown && (
                <div className="absolute top-full left-0 w-full bg-white border-grey overflow-y-auto">
                  {options.map((option, index) => {
                    return (
                      <div
                        className={`py-1 text-sm px-4 hover:bg-[#8888F8] ${
                          selectedOption.id === option.id ? "bg-light-grey" : ""
                        }`}
                        onClick={() => {
